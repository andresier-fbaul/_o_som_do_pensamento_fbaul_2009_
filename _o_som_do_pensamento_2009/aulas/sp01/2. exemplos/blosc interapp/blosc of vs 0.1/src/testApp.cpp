...
void testApp::draw(){
	...
	if(calcimg){
		...
		//desenhar um quadrado preto semi-opaco, para ir apagando
		glColor4f(0.f,0.f,0.f,0.27f);
		ofRect(0,0,thiswidth,thisheight);
		
		// activar o aditive blend
		glBlendFunc(GL_SRC_ALPHA, GL_ONE); //additive blend
				
		cv.draw(0,0); 
	}
	
	// build some custom graphix
...
//	send numblobs first
	...
	for (int num = 0; num < numblobs; num++){
		...
		/// osc sending code
		ofxOscMessage m; //tem de ser outra mensagem!!
	...
	if(calcimg){
		...
		if(stattxt == 1){
			sprintf(reportStr, "blosc // andre sier // s373.net");
			ofDrawBitmapString(reportStr, 70,10);		
		} else if(stattxt==3){
			sprintf(reportStr, "sends /blosc/num <numblobs>");
			ofDrawBitmapString(reportStr, 77,10);			
		} else if(stattxt==5){
			sprintf(reportStr, "sends /blosc/data <id> <x> <y> <area>");
			ofDrawBitmapString(reportStr, 50,10);
		} else if(stattxt==7){
			sprintf(reportStr, "press space to clear background (om 0)");
			ofDrawBitmapString(reportStr, 50,10);			
		} else if(stattxt==9){
			sprintf(reportStr, "press o to change om (om 0=bg; om 1=dif)");
			ofDrawBitmapString(reportStr, 50,10);			
		}
	}
}
