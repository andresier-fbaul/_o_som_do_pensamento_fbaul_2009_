/*  som do pensamento, desenhar_1a
 ...
 agora o escriva encaracola da posição em que é lançado
...
void draw(){
  ...
  if(mousePressed){
    ...
    if(random(1)<0.7) // 70% de hipoteses de ser activado
      if(escriva.energy <= 0.){
...
void keyPressed(){
 if (key == 's')
   saveFrame("desenhar-1a-####.jpg");//save();
 if (key == ' ')
   background(150);
