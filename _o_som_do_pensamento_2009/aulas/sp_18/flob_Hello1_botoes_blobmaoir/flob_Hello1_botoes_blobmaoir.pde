...
void setup(){
  ...
  flob.setThresh(TRESH); //set the new threshold to the binarize engine

  flob.setBackground(video); // zero background to contents of video
  flob.setOm(flob.CONTINUOUS_DIFFERENCE);//CONTINUOUS_DIFFERENCE
 ...
 // b2 = new Botao ( 150, 300, 120, 1   );
  ...
  for(int i=0; i<botoes.length;i++){
     botoes[i] = new Botao(random(width),random(height),random(50,150), (int)random(2)); 
  }
...
//       botoes[j].test(center[0],center[1]); 
//    }
//
...
//    b2.test(center[0],center[1]);
  ...
  if(flob.getNumBlobs() > 0 ) {
  
     // inserir teste espacial
    center = flob.getCentroid(indexmaior);
    center[0] = center[0] * width;
    center[1] = center[1] * height;
