...
void drawSlide(PImage slide, float fadeFactor){
  if(slide == null)
    return;
...
// will the impossible
    if(random(0,1) < 0.00005) translate(0,random(0,1) * height/8);
// error happen?
  
    float inFadeFactor = inSlideTime / slideFadeDur;
    drawSlide(thisSlide, 1.0-inFadeFactor);
