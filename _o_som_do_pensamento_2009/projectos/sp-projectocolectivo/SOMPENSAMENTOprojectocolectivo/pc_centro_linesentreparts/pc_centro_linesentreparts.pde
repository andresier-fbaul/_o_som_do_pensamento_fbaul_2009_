...
public void draw() {  
  ...
  MYPART2D part1,part2;

  for (int i = 0; i < particulas.size(); i+=2) {
    part1 = (MYPART2D) particulas.get(i);
    int index = i+1;
    if(index >= particulas.size()) {      
      part2 = new MYPART2D(width/2,height/2);            
    } 
    else {
      part2 = (MYPART2D) particulas.get(i+1);
      part2.update();      
    }
    part1.update();    
    //   part2.update();    
    ...
    rect(part1.x,part1.y, 2, 2);
    rect(part2.x,part2.y, 2, 2);
