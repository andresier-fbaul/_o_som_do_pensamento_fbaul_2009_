...
void setup() {
  ...
  burstImg = loadImage("pacmanBurst.png");
  pac = new Pacman(pacImg,burstImg);
  imgPart = loadImage("particle.png");
  // Start with 10 balls
  //  balls = new  ArrayList();
  particulas = new  ArrayList();
  for (int i = 0; i < 10; i++) {
    MYPART2D part= new MYPART2D(imgPart,random(width),random(height)); /// particulas gerados em toda a cena
    particulas.add(part);
  }
...
public void draw() {  
  ...
  pac.update(particulas);
  if (pac.burstMode == true)
  {
     MYPART2D part = new MYPART2D(imgPart,pac.x,pac.y);
     part.velx = 30*cos(pac.angle);
     part.vely = 30*sin(pac.angle);
     particulas.add(part);
  }
  pac.draw();
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if(theOscMessage.checkAddrPattern(particula2right)==true) {
    MYPART2D part = new MYPART2D(imgPart,0.f,0.f);
...
public void mousePressed() {
  ...
  MYPART2D part = new MYPART2D(imgPart,mouseX,mouseY);
  particulas.add(part);
